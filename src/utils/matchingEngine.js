...
const isTimestampMatch = (
    ...
    const diffInSeconds = Math.abs(new Date(timestamp1) - new Date(timestamp2))/1000;
...
const reconcileTransactions = (
    ...
    for (const userTx of userTransactions){
        ...
        for(const exchangeTx of exchangeTransactions){
            ...
            const timestampMatch = isTimestampMatch(userTx.timestamp, exchangeTx.timestamp);

            const quantityMatch = isQuantityMatch(userTx.quantity, exchangeTx.quantity);
...
export {isTimestampWithinTolerance, isQuantityWithinTolerance};
